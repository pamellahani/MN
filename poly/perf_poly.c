...
int main (int argc, char **argv)
{
  ...
  */
  ...
  // multiplication_scalaire (p4 *3)
  start = _rdtsc () ;
  
        p6 = multiplication_polynome_scalaire (p4, 3) ;

  end = _rdtsc () ;

  printf ("addition %Ld cycles\n", end-start) ;
  calcul_flop ("p4*3", p4->degre+1, end-start) ;
  detruire_polynome (p6) ;
  


  // multiplication par un polynome (p4 * p5)
  start = _rdtsc () ;
  ...
  printf ("addition %Ld cycles\n", end-start) ;
  calcul_flop ("p4*p5", p4->degre*p5->degre, end-start) ;
  detruire_polynome (p6) ;



    // puissance d'un polynome (p5^3)
  start = _rdtsc () ;
  
        p6 = puissance_polynome(p5,3) ;

  end = _rdtsc () ;

  printf ("addition %Ld cycles\n", end-start) ;
  calcul_flop ("p5^3", p5->degre+1, end-start) ;
  detruire_polynome (p6) ;
  

  //composition par un polynome (p4 o p5)
  start = _rdtsc () ;
  
        p6 = composition_polynome (p4, p5) ;

  end = _rdtsc () ;

  printf ("addition %Ld cycles\n", end-start) ;
  calcul_flop ("p4 o p5", p4->degre*p5->degre ,end-start); 
  detruire_polynome (p6) ;
}